...
import {
  ...
  scoreChunk,
  tooShortChunk,
} from '@slop-alarm/core';
import type { Answers, ChunkResult, JevErrorCode, JevProvider, ProviderConfig, TextChunk } from '@slop-alarm/core';
...
function repoRoot(): string {
  // tools/eval/src/run.ts -> tools/eval/src -> tools/eval -> tools -> repo root
  return resolve(here(), '../../..');
}
...
function parseArgs(argv: string[]): Args {
  ...
  const raw = dirArg ?? join(repoRoot(), 'eval/samples');
  // Resolve relative paths against the repo root, not process.cwd(): `npm run eval -w
  // @slop-alarm/eval` runs with cwd=tools/eval, so a relative arg typed at the repo root must not
  // resolve against that.
  const dir = isAbsolute(raw) ? raw : resolve(repoRoot(), raw);
  return { dir, refresh, jsonPath, provider };
}

const ENV_KEY_NAME: Record<JevProvider, string> = { typesafe: 'TYPESAFE_API_KEY', openrouter: 'OPENROUTER_API_KEY' };

/**
 * Picks a provider and key from `--provider` / the environment. With no key available at all, this
 * does NOT throw: it warns once and falls back to provider "typesafe" with an empty key, so a
 * keyless run can still proceed entirely off the answer cache (see src/cache.ts). Any chunk that
 * isn't already cached then fails with askJev's own "missing_key" error, same as it always did.
 */
export function resolveProvider(argProvider: JevProvider | undefined): ProviderConfig {
  if (argProvider) {
    const apiKey = process.env[ENV_KEY_NAME[argProvider]];
    if (!apiKey) throw new Error(`--provider ${argProvider} was given but ${ENV_KEY_NAME[argProvider]} is not set (add it to tools/eval/.env).`);
  ...
  console.warn('No TYPESAFE_API_KEY or OPENROUTER_API_KEY set: using cached responses only. Add a key to tools/eval/.env to query Jev.');
  return { provider: 'typesafe', apiKey: '' };
}

async function main(): Promise<void> {
  const { dir, refresh, jsonPath, provider: argProvider } = parseArgs(process.argv.slice(2));
  ...
  const cachePath = join(dir, '.cache', 'answers.json');
  ...
  printReport(report);
  writeResultsCsv(outcome.rows, join(dir, '.cache', 'results.csv'));
  console.log(`\nPer-sample results written to ${join(dir, '.cache', 'results.csv')}`);
  ...
  writeChunksCsv(outcome.chunkRows, join(dir, '.cache', 'chunks.csv'));
  console.log(`Per-chunk results written to ${join(dir, '.cache', 'chunks.csv')}`);
  if (jsonPath) {
