/**
 * Fits the scoring formula in packages/core/src/scoring.ts from the eval harness output.
 *
 *   npm run eval                     fills eval/samples/.cache/chunks.csv (answers are cached)
 ...
 * twin from eval/samples/ai/SOURCES.tsv, so neither a sibling chunk nor a twin text can leak.
 *
 * Flags: --features a,b,c   --l2 0.1   --bands 0.3,0.75   --csv <path>
 */
...
const csvPath = arg('--csv', path.join(here, '../../eval/samples/.cache/chunks.csv'));
...
if (!fs.existsSync(csvPath)) {
  console.error(`Missing ${csvPath}. Run: npm run eval -- eval/samples`);
  process.exit(1);
}
...
const sources = path.join(here, '../../eval/samples/ai/SOURCES.tsv');
